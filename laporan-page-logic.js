...
function renderLaporanPage() {
        ...
        <div class="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm animate-pulse">
            ...
            <div class="space-y-3">
                <div class="grid grid-cols-3 gap-3">
                    <div class="bg-gray-200 p-2 rounded-lg h-12"></div>
                    <div class="bg-gray-200 p-2 rounded-lg col-span-2 h-12"></div>
                </div>
    ...
    setTimeout(() => {
        ...
        if (paginatedData.length === 0) {
            html = `<div class="md:col-span-2 lg:col-span-3 text-center py-16 bg-white rounded-2xl border border-dashed"><p class="text-gray-400 italic">Tidak ada data laporan untuk ditampilkan.</p></div>`;
        } else {
            paginatedData.forEach((s) => {
                ...
                const borderClass = isChecked ? 'border-primary ring-2 ring-primary/20 shadow-md' : 'border-gray-100 shadow-sm';
                ...
                html += `<div class="bg-white rounded-2xl p-5 border ${borderClass} transition-all duration-200 active:scale-[0.98] active:bg-gray-50/50 group flex flex-col relative"><div class="absolute top-5 left-5 z-10"><input type="checkbox" ${isChecked ? 'checked' : ''} onchange="toggleLaporanSelection('${s.id}', '${s.kategori}', this.checked)" class="w-4 h-4 rounded border-gray-300 text-primary focus:ring-primary cursor-pointer"></div><div class="flex items-start justify-between mb-4 flex-1 pl-7"><div class="flex items-center gap-3"><div class="w-10 h-10 rounded-full ${s.warna} flex items-center justify-center font-bold text-sm shrink-0">${s.inisial}</div><div><h4 class="font-bold text-teal-950 leading-tight">${s.nama}${badgeSusulan}</h4><p class="text-xs text-gray-500">${s.id} • ${s.kelas}</p></div></div><span class="px-2 py-0.5 text-[10px] font-bold rounded-md border ${statusBadge}">${statusText}</span></div><div class="space-y-3"><div class="grid grid-cols-3 gap-3 text-center"><div class="bg-gray-50 p-2 rounded-lg"><p class="text-[9px] font-bold text-gray-400 uppercase">Rata-rata</p><p class="text-lg font-black ${avgColor}">${avgText}</p></div><div class="bg-gray-50 p-2 rounded-lg col-span-2"><p class="text-[9px] font-bold text-gray-400 uppercase">Predikat</p><p class="text-base font-bold ${predikatColor} flex items-center justify-center gap-1">${predikatText}</p></div></div><div class="flex items-center justify-between text-xs text-gray-500 font-medium pt-3 border-t border-gray-100"><span class="font-bold text-blue-600">${s.kategori}</span><div class="flex items-center gap-1"><button onclick="bukaModalLaporanDetail('${s.id}', '${s.kategori}')" class="p-2 text-gray-400 hover:text-blue-500 transition-colors" title="Lihat Rincian Nilai"><span class="material-symbols-outlined text-base">summarize</span></button><button onclick="window.bagikanHasil('${s.id}', '${s.kategori}')" class="p-2 text-emerald-500 hover:text-emerald-700 transition-colors" title="Bagikan Link Hasil Ujian"><span class="material-symbols-outlined text-base">share</span></button><button ${!s.lulus ? 'disabled' : ''} onclick="cetakSyahadah('${s.id}', '${s.kategori}')" class="p-2 text-amber-500 hover:text-amber-700 transition-colors disabled:text-gray-300 disabled:cursor-not-allowed" title="Cetak Syahadah"><span class="material-symbols-outlined text-base">workspace_premium</span></button></div></div></div></div>`;
            });
        }
        ...
        const selectAllChk = document.getElementById('checkbox-pilih-semua-laporan');
        if (selectAllChk) {
            selectAllChk.checked = paginatedData.length > 0 && paginatedData.every(s => selectedLaporanPeserta.some(item => item.id === s.id && item.kategori === s.kategori));
        }
...
function renderLaporanPagination() {
    ...
    let html = '';
    html += `<button onclick="changeLaporanPage(${currentPage - 1})" ${currentPage === 1 ? 'disabled' : ''} class="px-4 py-2 rounded-xl text-sm font-bold bg-white border border-gray-200 text-gray-600 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50 transition-colors shadow-sm">Sebelumnya</button>`;
    ...
    if (endPage < totalPages) {
        ...
        html += `<button onclick="changeLaporanPage(${totalPages})" class="w-10 h-10 flex items-center justify-center rounded-xl text-sm font-bold bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 transition-colors shadow-sm">${totalPages}</button>`;
    }

    html += `<button onclick="changeLaporanPage(${currentPage + 1})" ${currentPage === totalPages ? 'disabled' : ''} class="px-4 py-2 rounded-xl text-sm font-bold bg-white border border-gray-200 text-gray-600 disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50 transition-colors shadow-sm">Berikutnya</button>`;
