...
(function initMasterDataSync() {
    ...
    try {
        ...
        const localPembimbing = localStorage.getItem('munaqosyah_listPembimbing');
        if (localPembimbing && localPembimbing !== 'undefined') {
            const parsed = JSON.parse(localPembimbing);
            if (typeof listPembimbing !== 'undefined') { listPembimbing.length = 0; parsed.forEach(k => listPembimbing.push(k)); }
            if (typeof window.renderListPembimbingEdit === 'function') window.renderListPembimbingEdit();
            if (typeof window.renderPembimbingDropdown === 'function') window.renderPembimbingDropdown();
        }
        ...
        console.error("Gagal memuat data master dari local storage:", e);
    }
    ...
    const dbCheckInterval = setInterval(() => {
        if (typeof db !== 'undefined') {
            ...
            // Sinkronisasi Realtime Pembimbing
            db.collection('appData').doc('pembimbing').onSnapshot(doc => {
                if (doc.exists && doc.data().list) {
                    const arr = doc.data().list;
                    if (typeof listPembimbing !== 'undefined' && Array.isArray(listPembimbing)) {
                        listPembimbing.length = 0;
                        arr.forEach(k => listPembimbing.push(k));
                    }
                    localStorage.setItem('munaqosyah_listPembimbing', JSON.stringify(arr));
                    if (typeof window.renderListPembimbingEdit === 'function') window.renderListPembimbingEdit();
                    if (typeof window.renderPembimbingDropdown === 'function') window.renderPembimbingDropdown();
                }
            }, err => console.error('Error sync pembimbing:', err));
            ...
            db.collection('appData').doc('semester').onSnapshot(doc => {
                if (doc.exists && doc.data().list) {
                    ...
                    if (typeof listSemester !== 'undefined' && Array.isArray(listSemester)) {
                        ...
                        arr.forEach(k => listSemester.push(k));
                    }
            ...
            // Sinkronisasi Realtime Settings
...
window.renderKategoriDropdown = function () {
    ...
    const optionsHtml = listKategori.map(k => `<option value="${k.nama}">${k.nama}</option>`).join('');
    const optionsWithAllHtml = `<option value="">Semua Kategori</option>` + optionsHtml;

    const selects = [
        { id: 'kategori-select', html: optionsHtml, retain: true, stateSync: true },
        { id: 'filter-tabel-kategori', html: optionsWithAllHtml, retain: true },
        { id: 'dash-filter-kategori', html: optionsWithAllHtml, retain: true },
        { id: 'laporan-filter-kategori', html: optionsWithAllHtml, retain: true },
        { id: 'tambah-kategori', html: optionsHtml, retain: true },
        { id: 'edit-kategori-select', html: optionsHtml, retain: true },
        { id: 'nilai-massal-kategori', html: '<option value="">Pilih Kategori...</option>' + optionsHtml, retain: true }
    ];
    ...
    selects.forEach(s => {
        ...
        if (el) {
            ...
            if (s.stateSync && typeof window.currentState !== 'undefined' && window.currentState.kategori) {
                currentVal = window.currentState.kategori;
            }
            el.innerHTML = s.html;
            if (s.retain && currentVal && listKategori.some(k => k.nama === currentVal)) {
                el.value = currentVal;
            } else if (s.stateSync && listKategori.length > 0) {
                el.value = listKategori[0].nama;
                if (typeof window.currentState !== 'undefined') window.currentState.kategori = listKategori[0].nama;
            }
        }
    });
};
...
window.pindahUrutanKategori = async function (index, arah) {
    ...
    try {
        ...
        if (typeof getCurrentVisibleView === 'function') {
            if (getCurrentVisibleView() === 'peserta' && typeof window.renderTablePeserta === 'function') window.renderTablePeserta();
...
window.resetFormMateri = function () {
    ...
    const inputMassal = document.getElementById('input-materi-massal');
    if (inputMassal) inputMassal.value = '';
...
window.simpanSemester = async function () {
    ...
    if (!nama) {
        openAlert("Nama semester tidak boleh kosong.");
        return;
    }

    if (listSemester.some(s => s.toLowerCase() === nama.toLowerCase())) {
        openAlert("Nama semester sudah ada.");
        return;
    }

    const newSemesterList = [...listSemester, nama].sort();
    listSemester.length = 0;
    newSemesterList.forEach(s => listSemester.push(s));
    try {
        ...
        openAlert("Gagal menyimpan semester: " + e.message);
    }
}

window.hapusSemester = function (namaSemester) {
    openConfirm(`PERINGATAN: Menghapus semester "${namaSemester}" akan MENGHAPUS SEMUA DATA PESERTA DAN NILAI pada semester tersebut. Apakah Anda benar-benar yakin?`, async (confirmed) => {
        if (confirmed) {
            ...
            try {
                const batch = db.batch();
                batch.set(db.collection('appData').doc('semester'), { list: listSemester });
                
                Object.keys(dataPeserta).forEach(key => {
                    if (key.startsWith(`${namaSemester}_`)) {
                ...
                // Jika semester yang dihapus adalah yang aktif, pindah ke semester lain
                if (window.currentState.semester === namaSemester) {
                    const newSemester = listSemester.length > 0 ? listSemester[0] : null;
                    if (typeof window.pilihSemester === 'function') {
                        window.pilihSemester(newSemester);
                    } else {
                        window.currentState.semester = newSemester;
                        localStorage.setItem('currentSemester', newSemester);
                    }
                }

                window.renderSemesterEditList();
                if (typeof openAlert === 'function') openAlert(`Semester "${namaSemester}" dan semua data terkait berhasil dihapus.`);
                if (loadingOverlay) loadingOverlay.classList.add('opacity-0', 'pointer-events-none');
            } catch (e) {
                openAlert("Gagal menghapus semester: " + e.message);
                const loadingOverlay = document.getElementById('loading-overlay');
                if (loadingOverlay) loadingOverlay.classList.add('opacity-0', 'pointer-events-none');
            }
        }
...
window.renderTanggalUjianList = function () {
    ...
    if (dates.length === 0) {
        ...
        dates.sort().forEach(d => {
            ...
            const formatted = new Date(y, m - 1, day).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' });
            html += `
...
// --- Fitur Redaksi Pesan WhatsApp ---
window.bukaModalRedaksiWa = function () {
    const inputIndividu = document.getElementById('setting-input-wa-individu');
    const inputMassal = document.getElementById('setting-input-wa-massal');

    const defaultIndividu = "Assalamu'alaikum wr. wb.\n\nBerikut adalah hasil Ujian Munaqosyah untuk Ananda *{nama}* pada kategori *{kategori}*.\n\nSilakan klik tautan di bawah ini untuk melihat rincian nilai:\n{link}\n\nTerima kasih.";
    const defaultMassal = "Assalamu'alaikum wr. wb.\n\nBerikut adalah tautan hasil Ujian Munaqosyah:\n\n{list_peserta}\n\nTerima kasih.";

    if (inputIndividu) inputIndividu.value = (typeof appSettings !== 'undefined' && appSettings.waTemplateIndividu) ? appSettings.waTemplateIndividu : defaultIndividu;
    if (inputMassal) inputMassal.value = (typeof appSettings !== 'undefined' && appSettings.waTemplateMassal) ? appSettings.waTemplateMassal : defaultMassal;

    const modal = document.getElementById('modal-redaksi-wa');
    if (modal) modal.classList.replace('hidden', 'flex');
}

window.tutupModalRedaksiWa = function () {
    const modal = document.getElementById('modal-redaksi-wa');
    if (modal) modal.classList.replace('flex', 'hidden');
}

window.simpanRedaksiWa = async function () {
    const inputIndividu = document.getElementById('setting-input-wa-individu');
    const inputMassal = document.getElementById('setting-input-wa-massal');
    
    if (!inputIndividu || !inputMassal) return;

    const valIndividu = inputIndividu.value.trim();
    const valMassal = inputMassal.value.trim();

    if (!valIndividu || !valMassal) {
        if (typeof openAlert === 'function') openAlert("Template pesan tidak boleh kosong.");
        return;
    }

    try {
        await db.collection('appData').doc('settings').set({ 
            waTemplateIndividu: valIndividu,
            waTemplateMassal: valMassal
        }, { merge: true });

        if (typeof appSettings === 'undefined') window.appSettings = {};
        appSettings.waTemplateIndividu = valIndividu;
        appSettings.waTemplateMassal = valMassal;
        localStorage.setItem('munaqosyahSettings', JSON.stringify(appSettings));

        window.tutupModalRedaksiWa();
        if (typeof openAlert === 'function') openAlert("Redaksi pesan WhatsApp berhasil diperbarui.");
    } catch (e) {
        if (typeof openAlert === 'function') openAlert("Gagal menyimpan redaksi pesan: " + e.message);
    }
}
